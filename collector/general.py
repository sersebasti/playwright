...
def run_actions_check_reaction(page: Page, actions: List[Dict[str, Any]], reaction: Dict[str, Any]) -> bool:
    ...
    for idx, action in enumerate(actions, start=1):
        ...
        try:
            ...
            if action_type == "goto":
                ...
                print(f"[ACTION {idx}] goto url={url}")
                page.goto(
                    url,
                ...
                print(f"[ACTION {idx}] fill selector={selector} count={locator.count()}")
                ...
                selector = require_field(action, "selector", f"ACTION {idx}")
                locator = page.locator(selector)
                print(f"[ACTION {idx}] click selector={selector} count={locator.count()}")
                locator.first.wait_for(state="visible", timeout=action_timeout)
                locator.first.click()
                ...
                selector = require_field(action, "selector", f"ACTION {idx}")
                locator = page.locator(selector)
                print(f"[ACTION {idx}] wait_visible selector={selector} count={locator.count()}")
                locator.first.wait_for(state="visible", timeout=action_timeout)
            ...
            filename = f"action_fail_{idx}_{action_type}_{safe_name(action.get('selector', 'no_selector'))}.png"
            safe_screenshot(page, filename)
            raise
    ...
    if reaction_type == "element_present":
        try:
            ...
            locator = page.locator(selector)
            print(f"[REACTION] selector={selector} count={locator.count()} url={page.url}")
            ...
            filename = f"reaction_fail_{safe_name(reaction.get('selector', 'no_selector'))}.png"
            safe_screenshot(page, filename)
            return False
    else:
